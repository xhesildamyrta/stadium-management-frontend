...
const Signup=()=>{
    ...
    const [user, setUser] = useState({
      ...
      const {name,NID, email, password , phone} = user;
   ...
   async function  signup()
       {
        let result = await axios.post("http://dev-local.airalbania.com/client/register",user);
        setErrors('Registration Successful')
        setUser({name:"", NID:"", email:"",password:"" , phone:""}) // To Clear all fields
    ...
    return(
        <Grid>
            <Paper elevation={10} style={paperStyle}>
                ...
                <TextField label='NID' name="NID" value={NID} onChange={e => onInputChange(e)} placeholder='Enter NID' type='text' fullWidth required/>
                ...
                <TextField label='Phone'  name="phone" value={phone}  onChange={e => onInputChange(e)} placeholder='Enter phone number' type='text' fullWidth />
